...
function SampleComponent({ secondStepActive }: SampleComponentProps) {
  return (
    <View>
      <CopilotStep
        order={0}
...
it("allows to manage tooltip width", async () => {
  ...
  const tooltipWrapper = tree.root
    .findByType(CopilotModal)
    .findByType(MyTooltipComponent).parent;

  expect(tooltipWrapper.props.style).toMatchObject(customStyle);
});
